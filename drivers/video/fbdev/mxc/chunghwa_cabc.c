...
#define DRIVER_NAME "chunghwa-cabc"

static const struct of_device_id cabc_dt_ids[] = {
	{ .compatible = "chunghwa,cabc", },
	{ /* sentinel */ }
};
...
static int cabc_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "mydebug:%s: Entered\n",__func__);
	...
	for_each_child_of_node(np, pp) {
		...
		printk(KERN_ERR "%s:mydebug:node=%s,cabc_enable=%d,flags=%x\n"
			,__func__,np->name,cabc_enable,flags);
		...
		printk(KERN_ERR "%s:mydebug:node=%s,gpio_flag=%d\n"
			,__func__,np->name,gpio_flag);

		devm_gpio_request_one(&pdev->dev, cabc_gpio,
					gpio_flag, "chunghwa-cabc");
	}
	printk(KERN_ERR "mydebug:%s: Finish\n",__func__);
	return ret;
}
...
MODULE_AUTHOR("Sharpy.HU");
MODULE_DESCRIPTION("ChungHwa CABC driver");
MODULE_LICENSE("GPL");
